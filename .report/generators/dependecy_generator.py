...
class DependencyCheckSectionGenerator:
    ...
    def generate(self, pdf: FPDF, t: dict):
        ...
        if not vulnerable_deps:
            ...
            pdf.cell(200, 10, txt=t.get("no_vulnerable_dependencies", "No vulnerable dependencies found."), ln=True)
            return

        for idx, dep in enumerate(vulnerable_deps, start=1):
            file_name = dep.get("fileName", t.get("unknown_file", "Unknown File"))
            file_path = dep.get("filePath", t.get("unknown_path", "Unknown Path"))

            pdf.set_font("Arial", style="B", size=12)
            pdf.cell(0, 10, f"{t['dependency']} {idx}: {file_name}", ln=True)

            pdf.set_font("Arial", size=10)
            pdf.multi_cell(0, 6, f"{t['path']}: {file_path}")
            pdf.ln(1)
            ...
            for vuln in dep.get("vulnerabilities", []):
                ...
                pdf.cell(0, 6, f"{t['vulnerability']}: {name}", ln=True)
                pdf.set_font("Arial", size=10)
                pdf.cell(0, 6, f"{t['severity']}: {severity}", ln=True)
                if desc:
                    pdf.multi_cell(0, 6, f"{t['description']}: {desc}")

                for ref in references:
                    url = ref.get("url")
                    if url:
                        pdf.set_font("Arial", style="I", size=9)
                        pdf.multi_cell(0, 5, f"{t['reference']}: {url}")
